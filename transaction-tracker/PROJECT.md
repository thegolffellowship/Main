...
Added "Technical Debt & Known Concessions" section (now in `PROJECT.md`) covering:
...
- **Migrate to Supabase / PostgreSQL** — SQLite is a deliberate concession for the bridge phase.
  ...
  and proper schema versioning. See the "Technical Debt & Known Concessions" section above
...
**Status:** Bridge column `events_id INTEGER REFERENCES events(id)` added to `tgf_events`
with backfill via `_backfill_events_id_on_tgf_events()` (exact name → partial LIKE →
year-narrowed LIKE).
...
**Status:** `event_id INTEGER REFERENCES events(id)` columns added to all the above tables
with backfill at startup; string columns kept as denormalized display caches.
...
3. **event_id FK on all tables** — replace string event_name copies with a proper
   ...
   display cache only
4. **tgf_events → events bridge** — `events_id REFERENCES events(id)` already added;
   enforce that every tournament event maps to a main event record at insert time
