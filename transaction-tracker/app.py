...
def api_create_event():
    ...
    if event:
        # The booleans ride on update_event rather than widening
        ...
        if extras and event.get("id"):
            try:
                ...
                logger.exception("Could not set %s on new event",
                                 ", ".join(extras))
...
def api_handicap_send_bulk_email():
    ...
    # v2.487.0: the send itself lives in database.send_handicap_cards so
    # the closeout routine can run it unattended; this route is the
    # page's door to the same function.
    from email_parser.database import send_handicap_cards
    out = send_handicap_cards(
        ...
        sent_by=session.get("role", "unknown"))
    if out.get("error"):
        return jsonify({"error": out["error"]}), int(out.get("http") or 500)
    return jsonify(out)
