...
- `gg_points_standings` — persisted snapshot of Golf Genius points-race standings (race_key, rank, player_name, customer_id FK, tournaments, wins, total_points, points_behind, fetched_at). Lazy-created; replaced per race by `refresh_points_race_standings()`. Buy-in status is joined live from `season_contests` at read time, never stored. See `docs/claude/customers.md → Points Race — Buy-in Status`.
