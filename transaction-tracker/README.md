...
- **AI-powered parsing** — sends each "New Order" email to Claude, which extracts every field automatically. No brittle regex. Adapts if the store changes email format. Default model is Haiku; orders containing TGF MEMBERSHIP route to Sonnet (membership + event combo orders consistently mash up on Haiku).
...
- **Event management** — auto-detects events from transactions, tracks registrations, supports manual player additions, RSVP tracking, payment reminders, tee time planning with sunset advisor. Cancel/postpone with bulk credit/refund flow. Four event formats: 9 Holes, 18 Holes, 9/18 Combo, **27 Holes** (custom pricing rules + per-event game add-on).
- **Pairings generator** — interactive PAIRINGS tab on every event with Random (history-aware round-robin) and ABCD (handicap tier) modes. Seed/lock individual players, cart pairs, or full foursomes; click-to-swap; cart-pair coloring; partner-request honoring. Tee time slots auto-computed.
- **Customer identity** — canonical `customers`/`customer_emails` tables as single source of truth. 5-step lookup cascade resolves returning customers by email, alias, or name. Five canonical resolver helpers (`resolve_player_email/phone/name/chapter/status`) protect every customer-facing send path from typo'd / stale `items.*` snapshots. Drift on new orders is captured as a `parse_warning` and surfaced in the COO action-items banner. Roster import, alias management, customer merging.
- **Accounting & bank reconciliation** — flat `acct_transactions` ledger as single source of truth for every financial event. CSV/PDF bank statement import (Chase, Frost Bank, Venmo), auto-match by amount + date + description. **GoDaddy batch auto-match uses subset-sum** (exact recursive search with suffix-sum pruning, integer-cent tolerance) instead of summing every order in a wide window. Inline match queue in Ledger tab. Books-posted vs. bank-reconciled rendered as two distinct checks. Cash flow 90-day view. Refund methods include **PayPal** alongside Venmo / Zelle / Check / GoDaddy.
