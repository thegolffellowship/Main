...
- `email_parser/print_pack.py` — the event PRINT PACK (v2.465.0): Starter Sheet + Cart Signs + Divisions & Flights + Proximity bound into one PDF by WeasyPrint; `GET /events/<id>/print-pack.pdf`; evening-before routine 5–10 PM Central mails tomorrow's packs once per change (`print_pack_sent:<id>` hash); bridge `scoring-print-pack:<id>[|send]`. See events.md
