...
- `docs/claude/score-entry.md` (**PLAYER SCORE ENTRY, Track A, 2026-09-25 — on branch `claude/live-score-entry` pending Kerry's rule-3b OK**: phone-first `/member/score?t=<group link>`, gross only, keyed to customer_id, one scorer per group with explicit take-over (no timeout), offline queue idempotent on op_id, Foursomes team row; the event-scoped rounds-plural read Track B consumes; GG stays the money record — `test_score_entry.py` fails if anything but `email_parser/score_entry.py` touches se_*)
