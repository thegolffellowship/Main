...
- `email_parser/margin_ledger.py` — `lsc_fund_year` (Aug–Jul → that July's Cup), `margin_gaps` (pre-cutover events: booked vs residual-would-book with reasons; measure-only; bridge `scoring-margin-gaps`), `liability_buckets` (payouts owed, credits held, shirt fund by Cup year, tax reserve by month; bridge `scoring-liabilities`)
