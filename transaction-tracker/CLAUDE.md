...
- `docs/claude/session-prompt-2026-09-09-next.md` (**paste-in start prompt for the next session** — where things stand, the FIRST JOB (events 3306 + 3313 not closed out), what shipped 2026-09-08, and the open queue in priority order)
- `.claude/skills/event-closeout/SKILL.md` (**the post-event routine** — scorecard import, the FINAL GG pairing ingest that nobody remembers, payout verification, handicap differentials + the per-event handicap card email, contest sync, financial summary, recap. Phases are dependency-ordered; §OPEN lists what Kerry has not ruled on. Invoked by "close out" / "closeout")
