...
The `.mcp.json` at the repo root auto-configures it. Just restart Claude Code in this directory and you'll see the `tgf-transactions` server with 50 tools.
...
- **MCP Server** in `mcp_server.py` — 50 tools for Claude direct DB access
...
- `mcp_server.py` — MCP server (50 tools for Claude direct DB access)
