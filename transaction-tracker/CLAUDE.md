...
- `docs/claude/handoff-2026-09-15-event-night-leaderboard.md` (**session record, 2026-09-15 event night, v2.437.0→v2.456.0** — PAIRING RULE 15 (a credited/WD player leaves the sheet by himself; BLINDs drawn at random among the members with the fewest blinds this year; `blind_draws` table), the money hold (`_event_field_complete` — every hole of every player before a dollar or a win tint shows), the leaderboard wave (tee circles, all holes as columns, live skins, team handicap in PH, team cards on the chevron), the starter-sheet tee circles + NEW/1T badges, **the PLUS-HANDICAP MVP rule** (a plus comes off the round, never off a hole), the GG re-pull chain that had made both chapters' winnings wrong, Apple Pay + one-modal Mark Paid, and the handicap-card counts that would not reconcile. §7 is the TIMEZONE trap hit TWICE in one night; §10 carries the open rule-3b ratifications and the handicap-card identity bug)
