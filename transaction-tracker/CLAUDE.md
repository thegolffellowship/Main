...
- `docs/claude/handoff-2026-09-09-financial-audit.md` (**Finance lane session record, 2026-09-09 evening — READ-ONLY audit** — the money map (artifact linked inside), the nine ranked findings (five liability buckets have inflows only; a 170-row / $84K expense queue back to April; reconciliation abandoned since 7/28 with a book balance that is noise; income with no home; the returns unverifiable beyond March), the liability-ledger design of record (`fund_buckets` registry + `acct_transactions.fund_bucket` + `acct_allocations.pool_detail`, balance derived in one function, untagged-outflow queue), the one-monthly-close simplification list, filed-vs-Tracker sales tax, and the decision register D1–D9 put to Kerry one at a time. Nothing shipped; rule 3b gates all of it)
...
4. **Platform dialogue mailbox.** At the START of a session, read the mailbox for new posts (`read_platform_dialogue`, since the last id you've seen). **And before answering EVERY prompt (Kerry, 2026-09-10, verbatim: "Always check latest tracker mail before responding to any prompt. Don't necessarily need to include anything if it doesn't apply, but you need to check each time regardless.")** — a `since_id` read costs one call; another lane may have moved money, a dial or a ruling since your last turn. Mention what applies; stay silent on what does not; never skip the read. At the END of a substantive session, post a digest (author='tracker-claude'): what shipped, findings that affect Platform planning, open questions. Refresh `docs/claude/state-of-the-tracker.md` after major build waves so `get_tracker_docs` always serves a current brief.
